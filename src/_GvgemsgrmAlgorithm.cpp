...
arma::mat update_sigma_i(const arma::mat &omega,
  ...
  // # Update the ith sigma in E-step. 2024.04.30.
...
arma::rowvec update_mu_i(const arma::mat    &sigma_i,
  ...
  // # Update the ith mu in E-step. 2024.04.30.
...
double update_ksi_ij(const arma::rowvec &Aj,
  ...
  // # For computational simplicity, set ksi_{ij,y_{ij}} = 1 for y_{ij} = 0
  // # and set ksi_{ij,y_{ij}+1} = 1 for y_{ij} + 1 = R_{j}.
...
// [[Rcpp::export]]
arma::rowvec update_Aj_(const arma::Row<int> &Mj,
                        const arma::rowvec &Bj,
                        const int Rj,
...
// [[Rcpp::export]]
arma::rowvec update_Bj_(const arma::rowvec &Aj,
                        const arma::rowvec &Bj,
                        const int Rj,
                        const arma::mat &mu_,
                        const arma::vec &eta1_j,
...
double calcu_n2vlb_j(const arma::rowvec &Aj,
  ...
  // # Eyx3107, 2024.05.14
  // # Compute VBIC value for For certain j.
  // # -- Input --
  // # Aj        : K*1      vec, the jth row of A.
  // # Bj        : max_Rj*1 vec, the jth row of B, length of Bj maybe larger than Rj.
  // # mu_       : N*K      mat, mu for all i and k.
  ...
  //   
  // # -- Output --
  // # vbic_j: a numeric, variational bic value for certain j.
  //   
  // # -- Note --
...
// [[Rcpp::export]]
double calcu_entro_(const arma::cube &sigma_,
                    const int N,
                    const int K){
  int i;
  double entro = 0.0;
...
Rcpp::List vem_grm(
  ...
  // tol_vlb:  tolerance for variational lower bound, default is 1e-4.
  // stop_cri: stopping criterion, 1 denotes only vlb, 2 denotes only paras, 3 denotes both vlb & paras.
  ...
  calcu_eta_(old_eta1, old_ksi1, y, z, R, 1);
  calcu_eta_(old_eta2, old_ksi2, y, z, R, 2);
  ...
  while(it < max_iter){
    ...
    calcu_eta_(new_eta1, new_ksi1, y, z, R, 1);
    calcu_eta_(new_eta2, new_ksi2, y, z, R, 2);

    // -- 2.2 update B --
    for(j=0;j<J;j++){
      new_B.row(j) = update_Bj_(old_A.row(j), old_B.row(j), R(j), mu_n,
                                new_eta1.col(j), new_eta2.col(j), y.col(j));
    }
    
    // -- 2.3 update A --
    for(j=0;j<J;j++){
      new_A.row(j) = update_Aj_(Mod.row(j), new_B.row(j), R(j), y.col(j),
    ...
    if(converge_para == 1){
      ...
      entro   = calcu_entro_(sigma_n, N, K);
      n2vlb = n2vlb_0 + sum(n2vlb_vec) + entro;
  ...
  List output = List::create(Rcpp::Named("new_A")    = new_A,
                             ...
                             Rcpp::Named("n2vlb") = n2vlb,
                             Rcpp::Named("it")    = it,
                             
                             // Rcpp::Named("converge_vlb")  = converge_vlb,
