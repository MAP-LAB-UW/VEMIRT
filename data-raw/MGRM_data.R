...
generate_para <- function(true_Mod, D, J, true_K){
  
  a <- matrix(runif(J*D,0.5,2), J, D)
  true_A <- a * true_Mod
  true_A <- round(true_A,3)
  
  mat <- matrix(0, J, (max(true_K)))
  for(j in 1:J)
  {
    mat[j,] <-  c(rep(1, (true_K[j])), rep(0, (max(true_K)- true_K[j])))
  }
  if(max(true_K) != 1){
    # a vector including the number of graded score for all items
    true_gam <- matrix(data=runif(J*(max(true_K)),.3,1), nrow=J, ncol=max(true_K))
    true_B   <- t(scale(apply(true_gam, 1, cumsum), center=TRUE, scale=FALSE))
    attr(true_B, "scaled:center") <- NULL
    true_B <- round(true_B,3)
    true_B <- true_B* mat
  }else
    true_B <- runif(J,0.5,2)
...
generate_data_function <- function(N,A,B,Sig,K)
{
...
#demo
set.seed(243)
res1 <- generate_para(true_Mod, D, J, true_K = K_list) # generate parameters
res2 <- generate_data_function(N, res1$a_true, res1$b_true, res1$Sig_true, K = K_list) # generate responses
MGRM_data <- list(data = res2$y, model = true_Mod, params = list(a = res1$a_true, b = res1$b_true, theta = res2$x))
