...
init.D2PL_bifactor_gvem <- function(Y, D, k, impact, X, iter, eps, ...) {
  N <- nrow(Y)
  ...
  gamma.mask <- a.mask$clone()
  gamma.mask[, (k + 1):K] <- 0
  gamma.mask <- torch_stack(replicate(G, gamma.mask)) * group.mask$unsqueeze(3)
  gamma.mask[1] <- 0
  gamma <- gamma.mask * 0
  ...
  lambda <- 0
  niter.init <- mstep.D2PL_bifactor_gvem()
  keep(Y, D, k, impact, X, iter, eps, N, n, J, K, G, MU, SIGMA, Mu, Sigma, a, b, gamma, beta, Y.mask, a.mask, gamma.mask, beta.mask, group.mask, eta, niter.init)
  init.new()
}

mstep.D2PL_bifactor_gvem <- function() {
  with(parent.frame(), {
    params.old <- NULL
    for (i in 1:iter) {
      ...
      if (impact)
        diagonal(Sigma)$copy_(diagonal(sigma))
      Sigma[, 1:k, 1:k] <- sigma[, 1:k, 1:k]
      ...
      a <- (pinverse((2 * eta$view(c(N, -1, 1, 1)) * sigma.mu)$sum(1)) %*% ((Y - 0.5)$view(c(N, -1, 1, 1)) * mu + 2 * eta$view(c(N, -1, 1, 1)) * (BB$view(c(N, -1, 1, 1)) * mu - sigma.mu %*% gamma[X]$unsqueeze(4)))$sum(1))$squeeze(3)$masked_fill(!a.mask, 0)
      ...
      gamma.beta  <- torch_stack(lapply(n, function(n) {
        ...
        sigma.mu <- sigma.mu[n]
        if (lambda == 0)
          torch_cat(list((pinverse((2 * eta$view(c(-1, J, 1, 1)) * sigma.mu)$sum(1)) %*% ((Y - 0.5)$unsqueeze(3) * mu$squeeze(4) + 2 * eta$unsqueeze(3) * (BB[n]$unsqueeze(3) * mu$squeeze(4) - (sigma.mu %*% a$unsqueeze(3))$squeeze(4)))$sum(1)$unsqueeze(3))$squeeze(3),
...
est.D2PL_bifactor_gvem <- function(e, lambda) {
  list2env(e, environment())
  niter <- if (lambda == 0)
    c(0, 0)
  else {
    j <- mstep.D2PL_bifactor_gvem()
    lambda <- 0
    gamma.mask <- gamma != 0
    beta.mask <- beta != 0
    c(j, mstep.D2PL_bifactor_gvem())
  }
  ...
  final.D2PL_bifactor_gvem()
}

init.D2PL_bifactor_iwgvemm <- function(Y, D, k, impact, X, iter, eps, S, M, lr, ...) {
  init <- do.call(init.D2PL_bifactor_gvem, as.list(environment()))
  e <- init()
  ...
  Sigma1.L.mask <- torch_block_diag(list(lower.tri(matrix(0, k, k)), matrix(0, K - k, K - k)))$bool()
  Sigma1.L.v <- (Sigma.L[1, 1:k, 1:k] / torch_cat(list(torch_ones(k, 1), sqrt(1 - Sigma.L[1, 1:k, 1:k]$square()$cumsum(2))[, 1:-2]), 2))$masked_select(Sigma1.L.mask[1:k, 1:k])$arctanh()$requires_grad_(T)
  if (G == 1) {
    Sigma2.L.mask <- torch_empty(c(0, K, K), dtype = torch_bool())
    Sigma2.L.v <- torch_empty(0, requires_grad = T)
  } else {
    Sigma2.L.mask <- torch_block_diag(list(lower.tri(matrix(0, k, k), T), diag(rep(impact, K - k))))$bool()$unsqueeze(1)$expand(c(G - 1, -1, -1))
  ...
  lambda <- 0
  niter.init <- c(init('niter.init'), mstep.D2PL_bifactor_iwgvemm())
  keep(Y, D, k, impact, X, iter, eps, S, M, lr, Y.mask, X.rank, N, n, J, K, G, SIGMA, MU, Sigma.L, Sigma1.L.mask, Sigma1.L.v, Sigma2.L.mask, Sigma2.L.v, Mu, Mu.mask, Mu.v, a, a.mask, a.v, b, gamma, gamma.mask, gamma.v, gamma.v.mask, beta, beta.mask, beta.v, beta.v.mask, theta, theta.logd, Q, niter.init,)
  init.new()
}

assemble.D2PL_bifactor_iwgvemm <- function() {
  with(parent.frame(), {
    z <- Sigma1.L.v$tanh()
    ...
    if (!impact)
      diagonal(Sigma2.L[, (k + 1):K, (k + 1):K])$fill_(1)
...
est.D2PL_bifactor_iwgvemm <- function(e, lambda) {
  list2env(e, environment())
  niter <- if (lambda == 0)
    c(0, 0)
  else {
    j <- mstep.D2PL_bifactor_iwgvemm()
    lambda <- 0
    gamma.v.mask <- gamma.v != 0
    beta.v.mask <- beta.v != 0
    c(j, mstep.D2PL_bifactor_iwgvemm())
  }
  with_no_grad({
    ll <- as.array(Q)
...
final.D2PL_bifactor_gvem <- function() {
  with(parent.frame(), {
...
#' GVEM Algorithms for DIF Detection in M2PL Bifactor Models
#'
...
#' @param ndim The first \code{ndim} factors are substantive, while others are nuisance factors
#' @param impact Whether nuisance factors are allowed to have impact
...
#'   \item{ ...$gamma}{D2PL_bifactor parameters for the slopes}
#'   \item{ ...$beta}{D2PL_bifactor parameters for the intercepts}
#'   \item{ ...$ll}{Estimated lower bound of log-likelihood}
#'   \item{ ...$l0}{Number of nonzero D2PL_bifactor parameters in \code{gamma} and \code{beta}}
...
#' @seealso \code{\link{D2PL_gvem}}, \code{\link{coef.vemirt_DIF}}, \code{\link{print.vemirt_DIF}}, \code{\link{summary.vemirt_DIF}}
#' @export
#'
#' @examples
#' \dontrun{
#' with(D2PL_bifactor_data, D2PL_bifactor_gvem(data, model, ndim, impact, group))}
D2PL_bifactor_gvem <- function(data, model = matrix(1, ncol(data)), ndim = ncol(model), impact = FALSE, group = rep(1, nrow(data)), method = 'IWGVEMM', Lambda0 = if (length(unique(group)) == 1) 0 else seq(0.1, 1, by = 0.1), criterion = 'GIC', iter = 500, eps = 1e-3, c = 1, S = 10, M = 10, lr = 0.1, verbose = TRUE) {
  fn <- switch(method,
               GVEM = list(init.D2PL_bifactor_gvem, est.D2PL_bifactor_gvem),
               IWGVEMM = list(init.D2PL_bifactor_iwgvemm, est.D2PL_bifactor_iwgvemm),
  ...
  init <- fn[[1]](Y, D, ndim, impact, X, iter, eps, S, M, lr, c)
