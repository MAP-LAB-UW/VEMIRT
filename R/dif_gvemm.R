...
EM.gvemm <- function(Y, D, X, lambda, iter, eps) {
  ...
  result <- if (lambda != 0) {
    ...
    c(pars, list(niter = niter))
  } else {
    i <- update()
    c(parameters(), niter = i)
  }
...
IW.iwgvemm <- function(Y, D, X, SIGMA, MU, Sigma, Mu, a, b, gamma, beta, lambda, z, theta.logd, S, M, iter, eps, lr) {
  ...
  update <- function() {
    with(parent.frame(), {
      opt <- optim_adam(list(Sigma1.L.v, Sigma2.L.v, Mu.v, a.v, b, gamma.v, beta.v), lr = lr, eps = 1e-3)
      ...
      for (i in 1:iter) {
        ...
        (-Q)$backward()
        if (lambda == 0) {
          gamma.v$grad$masked_fill_(!gamma.v.mask, 0)
          beta.v$grad$masked_fill_(!beta.v.mask, 0)
        }
        opt$step()
        if (lambda != 0)
          with_no_grad({
            proximal(gamma.v, opt$state$get(gamma.v), opt$param_groups[[1]], lambda)
            proximal(beta.v, opt$state$get(beta.v), opt$param_groups[[1]], lambda)
          })
        params <- parameters()
  ...
  result <- if (lambda != 0) {
    ...
    for (i0 in 1:iter) {
      ...
      i2 <- update()
      niter <- rbind(niter, c(i1, i2))
      print(c(i1, i2, sum(as.array(gamma.v.mask)), sum(as.array(beta.v.mask)), as.array(Q)))
      info <- lapply(list(gamma.v.mask, beta.v.mask, Q / N), as.array)
      if (!is.null(info.old) && all(distance(info, info.old) < eps))
        break
      info.old <- info
      lambda <- lambda0
    }
    c(parameters(), list(niter = niter))
  } else {
    i <- update()
    c(parameters(), niter = i)
  }
...
#' @param Lambda0 A vector of \code{lambda0} values for \eqn{L_1} penalty (\code{lambda} is \code{sqrt(N) * lambda0})
...
#'   \item{best}{The location of \code{fit} in \code{all}}
...
gvemm_DIF <- function(Y, D, X, method = 'IWGVEMM', Lambda0 = seq(0.2, 0.7, by = 0.1), criterion = 'GIC', iter = 1000, eps = 1e-3, c = 0.75, S = 10, M = 10, lr = 0.1) {
  ...
  if (!(criterion %in% c('AIC', 'BIC', 'GIC')))
    stop(paste0("Information criterion '", criterion, "' not supported."))
  if (is.character(X))
    X <- as.factor(X)
  if (!is.integer(X))
    X <- as.integer(X)
  ...
  if (method == 'GVEMM') {
    ...
    result <- lapply(Lambda0, function(lambda0) {
      ...
      list2env(EM.gvemm(Y, D, X, lambda, iter, eps), environment())
      list2env(as.list(IC(ll, l0, c, N)), environment())
      setTxtProgressBar(pb, pb$getVal() + 1)
      list(lambda0 = lambda0, lambda = lambda, niter = niter, SIGMA = SIGMA, MU = MU, Sigma = Sigma, Mu = Mu, a = a, b = b, gamma = gamma, beta = beta, ll = ll, l0 = l0, N = N, AIC = AIC, BIC = BIC, GIC = GIC)
    })
  } else if (method == 'IWGVEMM') {
    cat('Running GVEMM for initial values...\n')
    em <- EM.gvemm(Y, D, X, 0, iter, eps)
    ...
    result <- lapply(Lambda0, function(lambda0) {
      ...
      list2env(IW.iwgvemm(Y, D, X, SIGMA, MU, Sigma, Mu, a, b, gamma, beta, lambda, z, theta.logd, S, M, iter, eps, lr), environment())
      list2env(as.list(IC(ll, l0, c, N)), environment())
      setTxtProgressBar(pb, pb$getVal() + 1)
      list(lambda0 = lambda0, lambda = lambda, niter = list(em$niter, niter), SIGMA = SIGMA, MU = MU, Sigma = Sigma, Mu = Mu, a = a, b = b, gamma = gamma, beta = beta, ll = ll, l0 = l0, N = N, AIC = AIC, BIC = BIC, GIC = GIC)
    })
  }
  close(pb)
