...
init.D1PL_em <- function(Y, D, X, level, iter, eps, ...) {
  ...
  keep(Y, D, X, level, iter, eps, grid, N, n, J, K, G, U, Sigma, Mu, a, b, Y.mask, beta, beta.mask, niter.init)
  init.new()
}
...
estep.D1PL_em <- function() {
  with(parent.frame(), {
    ...
    xi <- (((a * z$unsqueeze(-2))$sum(-1) - (b - beta)$unsqueeze(2))[X])$masked_fill(!Y.mask, NaN)
...
final.D1PL_em <- function() {
  with(parent.frame(), {
    ...
    c(lst(niter, ll, l0), lapply(lst(Sigma, Mu, a, b, beta), as.array), IC(ll, l0, N, c))
  })
}
...
#' @seealso \code{\link{D1PL_pair_em}}, \code{\link{D1PL_gvem}}, \code{\link{coef.vemirt_DIF}}, \code{\link{print.vemirt_DIF}}, \code{\link{summary.vemirt_DIF}}
#' @export
#'
#' @examples
#' \dontrun{
#' with(D1PL_data, D1PL_em(data, model, group))}
D1PL_em <- function(data, model = matrix(1, ncol(data)), group = rep(1, nrow(data)), method = 'EMM', Lambda0 = if (length(unique(group)) == 1) 0 else seq(0.1, 1, by = 0.1), level = 10, criterion = 'BIC', iter = 200, eps = 1e-3, c = 1, verbose = TRUE) {
  fn <- switch(method,
