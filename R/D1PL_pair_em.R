...
init.D1PL_pair_em <- function(Y, X, level, iter, eps, ...) {
  init <- do.call(init.D1PL_gvem, c(as.list(environment()), list(D = matrix(1, ncol(Y)))))
  e <- init()
  ...
  keep(Y, X, level, iter, eps, grid, Y.mask, group.mask, mask, N, n, J, G, U, Sigma, Mu, a, b, d.b, u.b, niter.init)
  init.new()
}

estep.D1PL_pair_em <- function() {
  with(parent.frame(), {
...
penalty.D1PL_pair_em <- function(d.u, a, mask) {
  sum((d.u - (a$unsqueeze(2) - a$unsqueeze(1)))$masked_fill(!mask, 0) ^ 2)
}

proximal.D1PL_pair_em <- function(d, u, a, tau, eta) {
...
mstep.D1PL_pair_em <- function() {
  with(parent.frame(), {
    eta <- lambda / rho
    b$requires_grad_(T)
    opt <- optim_lbfgs(lst(b), max_iter = iter, tolerance_change = eps)
    loss <- function() {
      xi <- ((a$unsqueeze(2) * z$unsqueeze(3) - b$unsqueeze(2))[X])$masked_fill(!Y.mask, NaN)
      Q <- (torch_where(U, nnf_logsigmoid(xi), nnf_logsigmoid(-xi))$nansum(3) * W)$sum() - rho / 2 * penalty.D1PL_pair_em(d.b + u.b, b, mask)
      opt$zero_grad()
      (-Q)$backward()
      -Q
    }
    ...
    for (ii in 1:iter) {
      opt$step(loss)
      with_no_grad({
        proximal.D1PL_pair_em(d.b, u.b, b, tau, eta)
        pars <- lapply(lst(b, d.b), torch_clone)
      })
...
count.D1PL_pair_em <- function(d, mask, G) {
  find <- function(x) {
    if (x != p[x])
      p[x] <<- find(p[x])
    else
...
em.D1PL_pair_em <- function() {
  with(parent.frame(), {
    j <- c()
    params.old <- NULL
    for (i in 1:iter) {
      estep.D1PL_pair_em()
      j <- c(j, mstep.D1PL_pair_em())
      params <- lapply(lst(Sigma, Mu, b, d.b), torch_clone)
...
init.lambda.D1PL_pair_em <- function(e, lambda, ...) {
  list2env(e, environment())
  tau <- Inf
  niter.init <- em.D1PL_pair_em()
  keep(Y, X, rho, level, iter, eps, c, grid, Y.mask, group.mask, mask, N, n, J, G, U, Sigma, Mu, a, b, d.b, u.b, P, niter.init)
  init.new()
}

est.D1PL_pair_em <- function(e, pb, lambda, Tau, ...) {
  init <- init.lambda.D1PL_pair_em(e, lambda)
  ...
  lapply(Tau, function(tau) {
    ...
    l0 <- sum(sapply(1:J, function(j) {
      result.b <- count.D1PL_pair_em(as.array(d.b[.., j]), as.array(group.mask[.., j]), G)
      d.b[.., j] <- result.b$d
      result.b$l0
    }))
    if (!is.null(pb)) setTxtProgressBar(pb, pb$getVal() + 1)
    c(lst(..., rho, lambda, tau, niter0 = init('niter.init'), niter, ll, l0), lapply(lst(Sigma, Mu, a, b, d.b, u.b), as.array), IC(ll, l0, N, c))
  })
}

#' EM Algorithm with ADMM for DIF Detection Using Group Pairwise Truncated \eqn{L_1} Penalty in 1PL Models
#'
...
#' @seealso \code{\link{D1PL_em}}, \code{\link{D1PL_gvem}}, \code{\link{coef.vemirt_DIF}}, \code{\link{print.vemirt_DIF}}, \code{\link{summary.vemirt_DIF}}
#' @export
#'
#' @examples
#' \dontrun{
#' with(D1PL_data, D1PL_pair_em(data, group, Tau = c(Inf, seq(0.01, 0.05, by = 0.01))))}
D1PL_pair_em <- function(data, group = rep(1, nrow(data)), Lambda0 = if (length(unique(group)) == 1) 0 else seq(0.5, 1.5, by = 0.1), Tau = if (length(unique(group)) == 1) 0 else c(Inf, seq(0.05, 0.3, by = 0.05)), rho0 = 0.5, level = 10, criterion = 'BIC', iter = 200, eps = 0.001, c = 1, verbose = TRUE) {
  Y <- as.matrix(data)
  N <- nrow(Y)
  X <- group
  if (is.character(X))
  ...
  rho <- m * rho0
  init <- init.D1PL_pair_em(Y, X, level, iter, eps, c, rho)
  ...
  results <- unlist(lapply(Lambda0, function(lambda0) {
    lambda <- m * lambda0
    lapply(est.D1PL_pair_em(init(), pb, lambda, Tau, rho0, lambda0), c)
  }), F)
  if (verbose) close(pb)
