...
init.D1PL_gvem <- function(Y, D, X, iter, eps, ...) {
  N <- nrow(Y)
  ...
  keep(Y, D, X, iter, eps, N, n, J, K, G, MU, SIGMA, Mu, Sigma, a, aa, b, beta, Y.mask, a.mask, beta.mask, group.mask, eta, niter.init, p, z0)
  init.new()
}
...
mstep.D1PL_gvem <- function() {
  with(parent.frame(), {
    ...
    for (i in 1:iter) {
      ...
      MU <- (SIGMA %*% (((Y - 0.5 + 2 * eta * BB)$unsqueeze(-1) * a)$sum(2)$unsqueeze(-1) + (Sigma.inv %*% Mu$unsqueeze(3))[X]))$squeeze(3)
      Mu <- groupmean(n, MU)
      mu <- MU - Mu[X]
      ...
      xi <- sqrt(BB$square() - 2 * BB * (a$unsqueeze(2) %*% mu)$view(c(N, -1)) + (a$unsqueeze(-2) %*% sigma.mu %*% a$unsqueeze(-1))$view(c(N, -1)))
...
init.D1PL_iwgvemm <- function(Y, D, X, iter, eps, S, M, lr, ...) {
  ...
  Sigma.L.mask <- torch_stack(replicate(G, lower.tri(matrix(0, K, K), T), F))$bool()
  Sigma.L.v <- Sigma.L$masked_select(Sigma.L.mask)$requires_grad_(T)
  ...
  keep(Y, D, X, iter, eps, S, M, lr, Y.mask, X.rank, N, n, J, K, G, SIGMA, MU, Sigma.L, Sigma.L.mask, Sigma.L.v, Mu, Mu.mask, Mu.v, a, b, beta, beta.mask, beta.v, beta.v.mask, theta, theta.logd, Q, niter.init, p, z0)
  init.new()
}

assemble.D1PL_iwgvemm <- function() {
  with(parent.frame(), {
    Sigma.L <- torch_zeros(Sigma.L.mask$shape)$masked_scatter(Sigma.L.mask, Sigma.L.v)
...
mstep.D1PL_iwgvemm <- function() {
  with(parent.frame(), {
    params <- lst(Sigma.L.v, Mu.v, b, beta.v)
...
final.D1PL_gvem <- function() {
  with(parent.frame(), {
    ...
    xi <- (a * theta$unsqueeze(-2))$sum(-1) - (b - beta)$unsqueeze(2)
    q <- (xi$sigmoid())$mean(2)
    RMSE <- sqrt(mean((p - q) ^ 2))
    l0 <- as.array(sum(beta != 0))
    c(lst(niter, ll, l0), lapply(lst(SIGMA, MU, Sigma, Mu, a, b, beta, RMSE), as.array), IC(ll, l0, N, c))
  })
}
...
#' @seealso \code{\link{D1PL_pair_em}}, \code{\link{D1PL_em}}, \code{\link{coef.vemirt_DIF}}, \code{\link{print.vemirt_DIF}}, \code{\link{summary.vemirt_DIF}}
#' @export
#'
#' @examples
#' \dontrun{
#' with(D1PL_data, D1PL_gvem(data, model, group))}
D1PL_gvem <- function(data, model = matrix(1, ncol(data)), group = rep(1, nrow(data)), method = 'IWGVEMM', Lambda0 = if (length(unique(group)) == 1) 0 else seq(0.1, 1, by = 0.1), criterion = 'GIC', iter = 200, eps = 1e-3, c = 1, S = 10, M = 10, lr = 0.1, verbose = TRUE) {
  fn <- switch(method,
